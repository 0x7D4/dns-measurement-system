...
from utils import load_dns_servers, get_ist_timestamp, get_system_dns_servers, get_system_hostname, get_public_ip, get_dhcp_server_ips
from ipwhois import IPWhois


def analyze_server(server_ip: str, isp_related_servers: Set[str], system_hostname: str, public_ip: Optional[str]) -> bool:
    """Analyze a single DNS server with isolated database connection."""
    db = None
    try:
        print(f"{datetime.now().strftime('%H:%M:%S')} Analyzing {server_ip}")
        ...
        isp_marker = " [ISP]" if is_isp_assigned else ""
        reliability_marker = f" [{result.test_reliability}]" if result.test_reliability != "RELIABLE" else ""
        latency = f"{result.latency_ms:.2f}ms" if result.latency_ms else "N/A"
        dnssec = "Yes" if result.dnssec_enabled else ("No" if result.dnssec_enabled is False else "N/A")
        blocking = "Yes" if result.malicious_blocking else ("No" if result.malicious_blocking is False else "N/A")
        
        print(f"{status}{isp_marker} {server_ip:43s}{reliability_marker}")
        print(f"  Latency: {latency:10s} DNSSEC: {dnssec:3s}  Blocking: {blocking:3s} Org: {result.organization[:20]}")
...
def get_whois_cache_stats() -> dict:
    ...
    try:
        ...
        print(f"Could not fetch WHOIS stats: {e}")
...
def enrich_whois_data_for_servers(dns_servers: List[str], max_lookups: int = 100) -> int:
    ...
    try:
        ...
        if private_ips_saved > 0:
            print(f"  Saved {private_ips_saved} private IP(s) with placeholder data")
        ...
        for idx, ip in enumerate(ips_to_process, 1):
            try:
                ...
                try:
                    ...
                    print(f"  Could not save error to cache: {save_err}")
                failed_count += 1
...
def record_localhost_identity(system_hostname: str, public_ip: Optional[str]) -> None:
    """
    Resolve WHOIS AS info for the local measurement host and store it in measurementhosts.
    This is executed at the very start of a cycle.
    """
    if not public_ip:
        print("Public IP is unknown, skipping host identity WHOIS.")
        return
    
    db = None
    try:
        print(f"[HOST] Recording identity for {system_hostname} ({public_ip})")
        ...
        org = rdap.get("network", {}).get("name") or rdap.get("asn_description") or "N/A"
        asn = rdap.get("asn") or "N/A"
        asndesc = rdap.get("asn_description") or "N/A"
        ...
        db.upsert_measurement_host(
            ...
            organization=org,
            asn=asn,
            asn_description=asndesc,
            country=country,
        )
    except Exception as e:
        print(f"Could not record host identity: {e}")
    finally:
        if db:
            try:
                db.close()
...
def run_analysis_cycle(dns_servers: List[str], delay: float) -> None:
    ...
    print(f"Analyzing {len(dns_servers)} servers (including {len(isp_related_servers)} ISP/DHCP-related)")
    print("=" * 80)
    
    # WHOIS cache stats from serveranalysisresults (for info only)
    whois_stats_before = get_whois_cache_stats()
    print("WHOIS Cache Stats (from analysis results):")
    ...
    print(f"\nChecking WHOIS data for {len(dns_servers)} DNS servers...")
    ...
    for idx, server_ip in enumerate(dns_servers, 1):
        ...
        if (idx % 10 == 0) or (idx == len(dns_servers)):
            ...
            print(f"Progress: {idx}/{len(dns_servers)} | Success: {successful} Failed: {failed} | ETA: {remaining:.0f}s")
    ...
    print(f"Success: {successful}/{len(dns_servers)} | Failed: {failed} | Time: {elapsed_time:.2f}s")
    print("=" * 80)
