...
if [ ! -f "/etc/systemd/system/${SERVICE_NAME}.service" ]; then
    ...
    # Create service file with proper Type and Restart policy
...
ExecStart=/bin/bash $SCRIPT_DIR/$REPO_DIR/run.sh
StandardOutput=append:$SCRIPT_DIR/analyzer.log
StandardError=append:$SCRIPT_DIR/analyzer.log
Restart=no
TimeoutStartSec=300
...
EOF
    ...
    echo "Systemd service and timer installed successfully" | tee -a "$LOG_FILE"
fi
